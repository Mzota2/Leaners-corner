...
import {BusinessData} from '../subjectData/businessData';
import './quiz.css'
import './history.css'
import { NavLink } from "react-router-dom";

 export default function BusinessQuiz(props){

  const [historyData, setData]=React.useState(BusinessData);
  ...
  function handleNext(){
    ...
    return(
      ...
      <div>
         {currentIndex>=BusinessData.length-1?<p className="score">Score:{score}</p>:console.log('we')}
      
         {currentIndex >=BusinessData.length-1?console.log('me'):<div onClick={handleAnswer}className="answer" id="answer">{answer.text}</div>}
    ...
    <main>
        ...
        <div className="questions_container">
            ...
            <NavLink to='/businessLessons'><p className="hs--goto">Go To Lessons<i class=" fab fas fa-angle-double-right"></i></p></NavLink>
