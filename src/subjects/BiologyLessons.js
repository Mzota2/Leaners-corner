...
import { BioLessonsData } from "../subjectData/biologyData";

import './history.css'

export default function BiologyLessons(){
    const [historyData , setHistoryData] = React.useState(BioLessonsData);
    ...
    function handleClick(){
        if(formData.level === 'all'){
            setHistoryData(BioLessonsData);
        }
       if(formData.level === 'form1'){
           
           setHistoryData(prevData =>{
                prevData = BioLessonsData;
