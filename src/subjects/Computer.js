...
import {ComputerData} from '../subjectData/computerData';
import './quiz.css'
import './history.css'
import { NavLink } from "react-router-dom";

 export default function ComputerQuiz(props){

  const [historyData, setData]=React.useState(ComputerData);
  ...
  function handleNext(){
      setCliked(false)
      setCurrentIndex(currentIndex+1);
      if(currentIndex >= ComputerData.length-1){
          setCurrentIndex(currentIndex);
    ...
    return(
      ...
      <div>
         {currentIndex>=ComputerData.length-1?<p className="score">Score:{score}</p>:console.log('we')}
      
         {currentIndex >=ComputerData.length-1?console.log('me'):<div onClick={handleAnswer}className="answer" id="answer">{answer.text}</div>}
    ...
    <main>
        ...
        <div className="questions_container">
            ...
            <NavLink to='/computerLessons'><p className="hs--goto">Go To Lessons<i class=" fab fas fa-angle-double-right"></i></p></NavLink>
