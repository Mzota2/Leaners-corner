...
import {MathematicsData} from '../subjectData/mathematicsData';
import './quiz.css'
import './history.css'
import { NavLink } from "react-router-dom";

 export default function MathematicsQuiz(props){

  const [historyData, setData]=React.useState(MathematicsData);
  ...
  function handleNext(){
      setCliked(false)
      setCurrentIndex(currentIndex+1);
      if(currentIndex >= MathematicsData.length-1){
          setCurrentIndex(currentIndex);
    ...
    return(
      ...
      <div>
         {currentIndex>=MathematicsData.length-1?<p className="score">Score:{score}</p>:console.log('we')}
      
         {currentIndex >=MathematicsData.length-1?console.log('me'):<div onClick={handleAnswer}className="answer" id="answer">{answer.text}</div>}
    ...
    <main>
        ...
        <div className="questions_container">
            ...
            <NavLink to='/mathematicsLessons'><p className="hs--goto">Go To Lessons<i class=" fab fas fa-angle-double-right"></i></p></NavLink>
