...
import { ChemLessonsData } from "../subjectData/chemistryData";

import './history.css'

export default function ChemistryLessons(){
    const [historyData , setHistoryData] = React.useState(ChemLessonsData);
    ...
    function handleClick(){
        if(formData.level === 'all'){
            setHistoryData(ChemLessonsData);
        }
       if(formData.level === 'form1'){
           
           setHistoryData(prevData =>{
                prevData = ChemLessonsData;
