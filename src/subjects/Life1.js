...
import {LifeData} from '../subjectData/lifeData';
import './quiz.css'
import './history.css'
import { NavLink } from "react-router-dom";

 export default function LifeQuiz(props){

  const [historyData, setData]=React.useState(LifeData);
  ...
  function handleNext(){
      setCliked(false)
      setCurrentIndex(currentIndex+1);
      if(currentIndex >= LifeData.length-1){
          setCurrentIndex(currentIndex);
    ...
    return(
      ...
      <div>
         {currentIndex>=LifeData.length-1?<p className="score">Score:{score}</p>:console.log('we')}
      
         {currentIndex >=LifeData.length-1?console.log('me'):<div onClick={handleAnswer}className="answer" id="answer">{answer.text}</div>}
    ...
    <main>
        ...
        <div className="questions_container">
            ...
            <NavLink to='/lifeLessons'><p className="hs--goto">Go To Lessons<i class=" fab fas fa-angle-double-right"></i></p></NavLink>
