...
import { BkLessonsData } from "../subjectData/bkData";

import './history.css'

export default function BkLessons(){
    const [historyData , setHistoryData] = React.useState(BkLessonsData);
    ...
    function handleClick(){
        if(formData.level === 'all'){
            setHistoryData(BkLessonsData);
        }
       if(formData.level === 'form1'){
           
           setHistoryData(prevData =>{
                prevData = BkLessonsData;
