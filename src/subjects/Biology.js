...
import {BioData} from '../subjectData/biologyData';
import './quiz.css'
import './history.css'
import { NavLink } from "react-router-dom";

 export default function BiologyQuiz(props){

  const [historyData, setData]=React.useState(BioData);
  ...
  function handleNext(){
      setCliked(false)
      setCurrentIndex(currentIndex+1);
      if(currentIndex >= BioData.length-1){
          setCurrentIndex(currentIndex);
    ...
    return(
      ...
      <div>
         {currentIndex>=BioData.length-1?<p className="score">Score:{score}</p>:console.log('we')}
      
         {currentIndex >=BioData.length-1?console.log('me'):<div onClick={handleAnswer}className="answer" id="answer">{answer.text}</div>}
    ...
    <main>
        ...
        <div className="questions_container">
            ...
            <NavLink to='/bioLessons'><p className="hs--goto">Go To Lessons<i class=" fab fas fa-angle-double-right"></i></p></NavLink>
