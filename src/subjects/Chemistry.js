...
import {ChemData} from '../subjectData/chemistryData';
import './quiz.css'
import './history.css'
import { NavLink } from "react-router-dom";

 export default function ChemistryQuiz(props){

  const [historyData, setData]=React.useState(ChemData);
  ...
  function handleNext(){
      setCliked(false)
      setCurrentIndex(currentIndex+1);
      if(currentIndex >= ChemData.length-1){
          setCurrentIndex(currentIndex);
    ...
    return(
      ...
      <div>
         {currentIndex>=ChemData.length-1?<p className="score">Score:{score}</p>:console.log('we')}
      
         {currentIndex >=ChemData.length-1?console.log('me'):<div onClick={handleAnswer}className="answer" id="answer">{answer.text}</div>}
    ...
    <main>
        ...
        <div className="questions_container">
            ...
            <NavLink to='/chemistryLessons'><p className="hs--goto">Go To Lessons<i class=" fab fas fa-angle-double-right"></i></p></NavLink>
