...
import { LifeLessonsData } from "../subjectData/lifeData";

import './history.css'

export default function LifeLessons(){
    const [historyData , setHistoryData] = React.useState(LifeLessonsData);
    ...
    function handleClick(){
        if(formData.level === 'all'){
            setHistoryData(LifeLessonsData);
        }
       if(formData.level === 'form1'){
           
           setHistoryData(prevData =>{
                prevData = LifeLessonsData;
