...
import {PhysicsData} from '../subjectData/physicsData';
import './quiz.css'
import './history.css'
import { NavLink } from "react-router-dom";

 export default function PhysicsQuiz(props){

  const [historyData, setData]=React.useState(PhysicsData);
  ...
  function handleNext(){
      setCliked(false)
      setCurrentIndex(currentIndex+1);
      if(currentIndex >= PhysicsData.length-1){
          setCurrentIndex(currentIndex);
    ...
    return(
      ...
      <div>
         {currentIndex>=PhysicsData.length-1?<p className="score">Score:{score}</p>:console.log('we')}
      
         {currentIndex >=PhysicsData.length-1?console.log('me'):<div onClick={handleAnswer}className="answer" id="answer">{answer.text}</div>}
    ...
    <main>
        ...
        <div className="questions_container">
            ...
            <NavLink to='/physicsLessons'><p className="hs--goto">Go To Lessons<i class=" fab fas fa-angle-double-right"></i></p></NavLink>
