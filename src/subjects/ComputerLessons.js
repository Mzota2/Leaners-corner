...
import { ComputerLessonsData } from "../subjectData/computerData";

import './history.css'

export default function ComputerLessons(){
    const [historyData , setHistoryData] = React.useState(ComputerLessonsData);
    ...
    function handleClick(){
        if(formData.level === 'all'){
            setHistoryData(ComputerLessonsData);
        }
       if(formData.level === 'form1'){
           
           setHistoryData(prevData =>{
                prevData = ComputerLessonsData;
